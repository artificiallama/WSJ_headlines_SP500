...
#The following stop_words are important for sentiment analysis especially
#words like 'up', 'down','under' are important for sentiment analysis of text related
#to stocks, finance, markets etc.
remove_words = ['above','below','up', 'down','off', 'over', 'under', 'few', 'more', 
                'most','own','against','during','out','too','very']
...
#Same as normalize_doc but takes care of abbreviations like
#U.S., U.K.
#normalize_doc() splits U.S. into U S. This method converts
#U.S. to usa and U.K. to UK before normalizing.
def normalize_doc_abbre(txt):
 txt = txt.lower()
 regex = re.compile(r'u.s')
 txt = regex.sub("usa",txt)
 regex = re.compile(r'u.k')
 txt = regex.sub("uk",txt)
